...
def compute_discriminator_loss(netD, real_RIRs, fake_RIRs,
    ...
    # Calculate loss for real pairs
    errD_real = criterion(real_features, real_labels)

    # Calculate loss for wrong pairs
    wrong_features = torch.cat((real_features[1:], real_features[0].unsqueeze(0)))
    errD_wrong = criterion(wrong_features, fake_labels[1:])

    # Calculate loss for fake pairs
    errD_fake = criterion(fake_features, fake_labels)
